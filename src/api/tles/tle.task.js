...
class TleTask {
  constructor(schedulePeriod) {
    this.tleTask = cron.schedule(
      schedulePeriod,
      async () => {
        ...
        try {
          ...
          const tleFile = await this.getTleFileFromSpaceTrack(date);
  ...
  async getTleFileFromSpaceTrack(date) {
    const cookie = await this.getLoginCookieFromSpaceTrack();
    const res = await requestPromise({
